...
To-do: 
  ...
  - add error margin to onscreen quantification
  - all variables lower case, only underscores in variable names
  - add plots
...
gaze_ip <- sample_data %>%
  mutate(
    onscreen =
      !is.na(Gaze_point_X) &
      !is.na(Gaze_point_Y) &
      Gaze_point_X >= 0 & Gaze_point_X < 1920 & #add error margin
      Gaze_point_Y >= 0 & Gaze_point_Y < 1080 #add error margin
  ) %>%
...
ip_summary <- ip_summary %>%
  mutate(
    include_trial =
      (is.na(IP1_duration) | IP1_gaze_time > 100) & #at least one fixation / 100 ms that are part of a fixation
...
prop_pupil_available <- data_pupil_bc_ds %>%
  ...
  summarise(
    prop_valid = mean(!is.na(pupil_ds)),#check
    .groups = "drop"
  )
